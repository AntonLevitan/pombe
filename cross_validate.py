...
def regression_cv(training_ontotype, training_data, rg_file, k_fold=6, cv_rg_filename="genotypes_rg.txt"):
    ...
    trainning_data_rg = training_data.values.ravel()
    ...
    for train_idx, test_idx in kf.split(training_ontotype):
        ...
        X_train, X_test, y_train, y_test = training_ontotype[train_idx], training_ontotype[test_idx], trainning_data_rg[train_idx], trainning_data_rg[test_idx]
        ...
        correlation_p.append(pearson[1])
        print("Pearsons correlation: {}, p-Value: {}".format(pearson[0], pearson[1]))
        rg_file.write("Test {}, Pearsons correlation: {}, p-Value: {}\r\n".format(i, pearson[0], pearson[1]))
        i += 1
    print("Pearsons mean correlation: {}, p-Value: {}".format(numpy.mean(correlation_r), numpy.mean(correlation_p)))
    rg_file.write("Pearsons mean correlation: {}, p-Value: {}\r\n".format(numpy.mean(correlation_r), numpy.mean(correlation_p)))
